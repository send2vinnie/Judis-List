...
using Blargo.JudisList.Tests.TestDatas;
...
namespace Blargo.JudisList.Tests
{
    ...
    /// Provides methods to easily work with <see cref="JudisListDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<JudisListTestModule>
    {
        ...
        protected void UsingDbContext(Action<JudisListDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<JudisListDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<JudisListDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<JudisListDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<JudisListDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JudisListDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<JudisListDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<JudisListDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<JudisListDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<JudisListDbContext, Task<T>> func)
        {
            T result;
